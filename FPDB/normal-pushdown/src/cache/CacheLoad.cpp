...
void CacheLoad::onCacheLoadResponse(const LoadResponseMessage &Message) { 
  ...
  if (useNewCacheLayout_) {
    ...
    // double partialHitNum=0.0;
    // double partialMissNum=0.0;
    ...
    // Caching only
      ...
      //partialHitNum=partialMissNum=1.0;
    }

    ctx()->send(CacheMetricsMessage::make(hitNum, missNum, shardHitNum, shardMissNum, 
            this->name()), "SegmentCache")
            .map_error([](auto err) { throw std::runtime_error(err); });
    // ctx()->send(CacheMetricsMessage::make(hitNum, missNum, shardHitNum, shardMissNum, 
    //         partialHitNum, partialMissNum,
    //         this->name()), "SegmentCache")
    //         .map_error([](auto err) { throw std::runtime_error(err); });
  }
