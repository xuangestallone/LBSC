...
normal::connector::MiniCatalogue::MiniCatalogue(
        ...
        //new
        std::shared_ptr<std::unordered_map<std::string, int>> tableIndex,
        std::shared_ptr<std::vector<std::unordered_map<std::string, int>>> columnsIndex,
        std::shared_ptr<std::unordered_map<std::shared_ptr<SegmentKey>, std::vector<size_t>, SegmentKeyPointerHash, SegmentKeyPointerPredicate>> segmentKeyToTableColumnPartitionIndex,
        ...
        new
        */
        tableIndex_(std::move(tableIndex)),
        columnsIndex_(std::move(columnsIndex)),
        segmentKeyToTableColumnPartitionIndex_(std::move(segmentKeyToTableColumnPartitionIndex)),
...
std::shared_ptr<std::unordered_map<std::string, int>> readMetadataSchemasForTableIndex(const std::string& schemaName) {
  auto res = std::make_shared<std::unordered_map<std::string, int>>();
  auto filePath = std::filesystem::current_path().append("metadata").append(schemaName).append("schemas");
  ...
  for (auto const &str: readFileByLines(filePath)) {
    auto splitRes = split(str, ":");
    res->emplace(splitRes[0],tableIndex);
    tableIndex++;
  }
  return res;
}

std::shared_ptr<std::vector<std::unordered_map<std::string, int>>> readMetadataSchemasForColumnIndex(const std::string& schemaName) {
  auto res = std::make_shared<std::vector<std::unordered_map<std::string, int>>>();
  auto filePath = std::filesystem::current_path().append("metadata").append(schemaName).append("schemas");
  for (auto const &str: readFileByLines(filePath)) {
    auto splitRes = split(str, ":");
    ...
    for (auto const &fieldEntry: split(splitRes[1], ",")) {
	    auto splitFieldEntry = split(fieldEntry, "/");
      fields.emplace(splitFieldEntry[0],columnIndex);
      columnIndex++;
    }
...
std::shared_ptr<std::unordered_map<std::shared_ptr<SegmentKey>, std::vector<size_t>, SegmentKeyPointerHash, SegmentKeyPointerPredicate>>
readMetadataSegmentInfoForIndex(const std::string& s3Bucket, const std::string& schemaName,
  std::shared_ptr<std::unordered_map<std::string, int>> tableIndex,
  std::shared_ptr<std::vector<std::unordered_map<std::string, int>>> columnsIndex
  ) {
  auto res = std::make_shared<std::unordered_map<std::shared_ptr<SegmentKey>, std::vector<size_t>, SegmentKeyPointerHash, SegmentKeyPointerPredicate>>();
  auto filePath = std::filesystem::current_path().append("metadata").append(schemaName).append("segment_info");
  for (auto const &str: readFileByLines(filePath)) {
    auto splitRes = split(str, ",");
    std::string objectName = splitRes[0]; 
    std::string column = splitRes[1]; 
    std::vector<size_t> indexs(3,0);
    auto spiltObjects = split(objectName,"/");
    auto tableAndPartitions = split(spiltObjects[spiltObjects.size()-1],".");
    auto tableName = tableAndPartitions[0];
    if(tableAndPartitions.size()==2){
      indexs[0] = 0;
    }else if(tableAndPartitions.size()==3){
      indexs[0] = std::stoi(tableAndPartitions[2]);
    }else{
      std::cout<<"readMetadataSegmentInfoForIndex error:"<<spiltObjects[spiltObjects.size()-1]<<std::endl;
      std::exit(-1);
    }
    auto iterTable = tableIndex->find(tableName);
    if(iterTable == tableIndex->end()){
      std::cout<<"cannot find table:"<<tableName<<std::endl;
      std::exit(-1);
    }
    size_t iTableIndex = iterTable->second;
    indexs[1] = iTableIndex;
    auto colunms = (*columnsIndex)[iTableIndex];
    auto iterColumn = colunms.find(column);
    if(iterColumn == colunms.end()){
      std::cout<<"cannot find column:"<<column<<std::endl;
      std::exit(-1);
    }
    indexs[2] = iterColumn->second;
...
std::shared_ptr<normal::connector::MiniCatalogue> normal::connector::MiniCatalogue::defaultMiniCatalogue(
  ...
  auto schemasForTableIndex = readMetadataSchemasForTableIndex(schemaName);
  auto schemasForColumnsIndex = readMetadataSchemasForColumnIndex(schemaName);
  ...
  auto sortedOrderKeyValues = std::make_shared<std::unordered_map<std::shared_ptr<Partition>, std::pair<std::string, std::string>,
          PartitionPointerHash, PartitionPointerPredicate>>();
  auto sortedOrderCommitdateValues = std::make_shared<std::unordered_map<std::shared_ptr<Partition>, std::pair<std::string, std::string>,
          PartitionPointerHash, PartitionPointerPredicate>>();
  ...
  for (int i = 0; i < (int) valuePairs->size(); i++) {
    sortedValues->emplace(std::make_shared<S3SelectPartition>(s3Bucket, s3ObjectDir + "lineorder" + fileExtension + "." + std::to_string(i)),
                          valuePairs->at(i));
    sortedOrderKeyValues->emplace(std::make_shared<S3SelectPartition>(s3Bucket, s3ObjectDir + "lineorder" + fileExtension +"." + std::to_string(i)),
                          make_pair(std::to_string(i), std::to_string(i)));
    sortedOrderCommitdateValues->emplace(std::make_shared<S3SelectPartition>(s3Bucket, s3ObjectDir + "lineorder" + fileExtension +"." + std::to_string(i)),
                          make_pair(std::to_string(i), std::to_string(i)));
  }
  sortedColumns->emplace("lo_orderdate", sortedValues);
  sortedColumns->emplace("lo_orderkey", sortedOrderKeyValues);
  sortedColumns->emplace("lo_commitdate", sortedOrderCommitdateValues);
  ...
  auto segmentKeyToTableColumnPartitionIndex = readMetadataSegmentInfoForIndex(s3Bucket, schemaName, schemasForTableIndex, schemasForColumnsIndex);
  ...
  return std::make_shared<MiniCatalogue>(schemaName, partitionNums, schemas, schemasForTableIndex, schemasForColumnsIndex, segmentKeyToTableColumnPartitionIndex,
    columnLengthMap, segmentKeyToSize, queryNumToInvolvedSegments, segmentKeysToInvolvedQueryNums, defaultJoinOrder, sortedColumns, csvFileDelimiter);
}
...
std::vector<size_t> normal::connector::MiniCatalogue::getSegmentSizegetSegmentTablePartitionColumnIndex(const std::shared_ptr<cache::SegmentKey>& segmentKey){
  auto key = segmentKeyToTableColumnPartitionIndex_->find(segmentKey);
  if (key != segmentKeyToTableColumnPartitionIndex_->end()) {
    return segmentKeyToTableColumnPartitionIndex_->at(segmentKey);
  }
  throw std::runtime_error("getSegmentSizegetSegmentTablePartitionColumnIndex Segment key not found in getSegmentSize: " + segmentKey->toString());
}
