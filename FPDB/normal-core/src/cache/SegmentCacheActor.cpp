...
void SegmentCacheActor::weight(const WeightRequestMessage &msg,
  ...
  if (cachingPolicy->id() == WFBR) {
	...
	auto bSCCachingPolicy = std::static_pointer_cast<BeladySizeCostCachingPolicy>(cachingPolicy);
	bSCCachingPolicy->onWeight(msg.getWeightMap(), msg.getQueryId());
  } else if(cachingPolicy->id() == LEARN) {
	auto learnCachingPolicy = std::static_pointer_cast<LearningCachingPolicy>(cachingPolicy);
	learnCachingPolicy->onWeight(msg.getWeightMap(), msg.getQueryId());
  }
}

// void SegmentCacheActor::metricsPartial(const CacheMetricsMessage &msg, 
// 				stateful_actor<SegmentCacheActorState> *self){
	
// 	//new
//   self->state.cache->addCrtQueryPartialHitNum(msg.getPartialHitNum());
//   self->state.cache->addCrtQueryPartialMissNum(msg.getPartialMissNum());

//     //new
//   self->state.cache->addPartialHitNum(msg.getPartialHitNum());
//   self->state.cache->addShardMissNum(msg.getPartialMissNum());
...
[[maybe_unused]] behavior SegmentCacheActor::makeBehaviour(stateful_actor<SegmentCacheActorState> *self,
  ...
  return {
	  ...
	  //new
	//   [=](GetNumPartialHitsAtom) {
	// 	return self->state.cache->partialHitNum();
	//   },
	//   [=](GetNumPartialMissesAtom) {
	// 	return self->state.cache->partialMissNum();
	//   },
	  ...
	  //new
	//   [=](GetCrtQueryNumPartialHitsAtom) {
	// 	return self->state.cache->crtQueryPartialHitNum();
	//   },
	//   [=](GetCrtQueryNumPartialMissesAtom) {
	// 	return self->state.cache->crtQueryPartialMissNum();
	//   },
	  ...
	  //new
	//   [=](ClearCrtQueryPartialMetricsAtom) {
	//   self->state.cache->clearCrtQueryPartialMetrics();
	//   },
	  [=](MetricsAtom, const std::shared_ptr<CacheMetricsMessage> &m) {
    	metrics(*m, self);
	  }
	//   [=](MetricsPartialAtom, const std::shared_ptr<CacheMetricsMessage> &m) {
    // 	metricsPartial(*m, self);
	//   }
  };
}
