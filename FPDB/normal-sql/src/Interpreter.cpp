...
Interpreter::Interpreter() :
  ...
  showQueryMetrics_(false),
  cachingPolicy_(LRUCachingPolicy::make())
{}

Interpreter::Interpreter(std::shared_ptr<normal::plan::operator_::mode::Mode> mode,
                         std::shared_ptr<CachingPolicy>  cachingPolicy,bool showQueryMetrics) :
  catalogues_(std::make_shared<std::unordered_map<std::string, std::shared_ptr<connector::Catalogue>>>()),
  mode_(std::move(mode)),
  showQueryMetrics_(showQueryMetrics),
...
std::string Interpreter::showMetrics() {
  ...
  for (size_t qid = 1; qid <= executionTimes_.size(); ++qid) {  
    ...
    if (stats.returnedBytes > 0 && stats.processedBytes > 0 &&
  ...
  for (size_t qid = 1; qid <= executionTimes_.size(); ++qid) {
    ...
    formattedReturnedGB << returnedGB << " GB"; //"Returned GB"
    if (processedGB > 0) {
      formattedReturnedPercentage << (returnedGB / processedGB) * 100.0 << "%"; //"Return %"
    } else {
    ...
    if (arrowOutputGB > 0) {
      formattedStorageFormatToArrowSizeX << returnedGB / arrowOutputGB << "x"; //"Storage/Compute Data Ratio"
      formattedConversionOutputRate << (double)arrowOutputMB /
              ((double)(stats.getConvertTimeNS + stats.selectConvertTimeNS) / 1.0e9) << " MB/s/req";// "Conv Output Rate";
    } else {
