...
void generateSqlFile(int argc, char **argv, std::filesystem::path &sql_file_dir_path) {
  ...
  switch (type) {
    ...
    case 6: {
      auto warmupBatch = sqlGenerator.generateSqlBatchSkewWeightForDP(skewness, size / 2);
      std::vector<std::string> executionBatch(warmupBatch);
      std::shuffle(warmupBatch.begin(), warmupBatch.end(), g);
      std::shuffle(executionBatch.begin(), executionBatch.end(), g);
      for (int index = 0; index < size; index++) {
    ...
    case 7: {
      auto warmupBatch = sqlGenerator.generateSqlBatchSkewWeightForBitMap(skewness, size / 2);
      std::vector<std::string> executionBatch(warmupBatch);
      std::shuffle(warmupBatch.begin(), warmupBatch.end(), g);
      std::shuffle(executionBatch.begin(), executionBatch.end(), g);
      for (int index = 0; index < size; index++) {
    ...
    case 8: {
      auto warmupBatch = sqlGenerator.generateSqlBatchSkewWeightForBitMapAndDP(skewness, size / 2);
      std::vector<std::string> executionBatch(warmupBatch);
      std::shuffle(warmupBatch.begin(), warmupBatch.end(), g);
      std::shuffle(executionBatch.begin(), executionBatch.end(), g);
      for (int index = 0; index < size; index++) {
