...
SqlGenerator::SqlGenerator() {
  ...
  skewBitMapAndDPQueryNameMap_.emplace("skewBitMapAndDPQuery0",SkewBitMapAndDPQuery0);
  skewBitMapAndDPQueryNameMap_.emplace("skewBitMapAndDPQuery2",SkewBitMapAndDPQuery2);
  skewBitMapAndDPQueryNameMap_.emplace("skewBitMapAndDPQuery3",SkewBitMapAndDPQuery3);
  skewBitMapAndDPQueryNameMap_.emplace("skewBitMapAndDPQuery4",SkewBitMapAndDPQuery4);
  skewBitMapAndDPQueryNameMap_.emplace("skewBitMapAndDPQuery5",SkewBitMapAndDPQuery5);

  skewBitMapQueryNameMap_.emplace("skewBitMapQuery5",SkewBitMapQuery5);
  skewBitMapQueryNameMap_.emplace("skewBitMapQuery2",SkewBitMapQuery2);
  skewBitMapQueryNameMap_.emplace("skewBitMapQuery3",SkewBitMapQuery3);
  skewBitMapQueryNameMap_.emplace("skewBitMapQuery4",SkewBitMapQuery4);

  skewDPQueryNameMap_.emplace("skewDPQuery0",SkewDPQuery0);
  skewDPQueryNameMap_.emplace("skewDPQuery2",SkewDPQuery2);
  skewDPQueryNameMap_.emplace("skewDPQuery3",SkewDPQuery3);
  skewDPQueryNameMap_.emplace("skewDPQuery4",SkewDPQuery4);
}
...
std::vector<std::string> SqlGenerator::generateSqlBatchSkewWeightForBitMapAndDP(float skewness, int batchSize) {
  // collect all skew query names
  std::vector<std::string> skewBitMapAndDPtQueryNames;
  for (auto const &queryNameIt: skewBitMapAndDPQueryNameMap_) {
    skewBitMapAndDPtQueryNames.emplace_back(queryNameIt.first);
  }
  ...
  // random queries
  std::uniform_int_distribution<int> distribution2(1, skewBitMapAndDPtQueryNames.size());
  ...
  for (size_t kind = 0; kind < possibilities.size(); kind++) {
    ...
    for (int i = 0; i < numThisKind; i++) {
      ...
      auto skewWeightQueryName = skewBitMapAndDPtQueryNames[skewWeightQueryIndex-1];
      queries.emplace_back(generateSqlSkewWeightForBitMapAndDP(skewWeightQueryName, skewLo_predicate, i % 2 == 0));
    }
  }
  ...
  if (queries.size() == batchSize) {
    ...
    while (queries.size() < batchSize) {
      ...
      int skewWeightQueryIndex = distribution2(*generator_);
      auto skewWeightQueryName = skewBitMapAndDPtQueryNames[skewWeightQueryIndex-1];
      queries.emplace_back(generateSqlSkewWeightForBitMapAndDP(skewWeightQueryName, skewLo_predicate, true));
    }
    return queries;
  }
}

std::string SqlGenerator::generateSqlSkewWeightForBitMapAndDP(std::string queryName, std::string skewLo_predicate, bool high){
  enum SkewBitMapAndDPQueryName queryNameEnum;
  auto queryNameIt = skewBitMapAndDPQueryNameMap_.find(queryName);
  if (queryNameIt == skewBitMapAndDPQueryNameMap_.end()) {
    queryNameEnum = SkewBitMapAndDPUnknown;
  } else {
    queryNameEnum = queryNameIt->second;
  }
  ...
  std::uniform_int_distribution<int> distribution_low(1,60000000);
  int low_orderkey = distribution_low(*generator_);
  
  while(low_orderkey>=60000000){
    low_orderkey = distribution_low(*generator_);
  }


  std::uniform_int_distribution<int> distributionForQuery5(0,9);
  int kind = distributionForQuery5(*generator_);
  std::string skewLo_predicateForQuery5 = fmt::format("lo_commitdate = {}", (1990 + kind) * 10000 + 131);

  switch (queryNameEnum) {
    case SkewBitMapAndDPQuery0: return genSkewWeightQuery0(low_orderkey);
    case SkewBitMapAndDPQuery2: return genSkewWeightQuery2(skewLo_predicate, lo_predicate, aggColumn);
    case SkewBitMapAndDPQuery3: return genSkewWeightQuery3(skewLo_predicate, lo_predicate, aggColumn);
    case SkewBitMapAndDPQuery4: return genSkewWeightQuery4(skewLo_predicate, lo_predicate, aggColumn);
    case SkewBitMapAndDPQuery5: return genSkewWeightQuery5(skewLo_predicateForQuery5);
    default:
      throw std::runtime_error("Unknown skew query name: " + queryName);
  }
}
...
std::vector<std::string> SqlGenerator::generateSqlBatchSkewWeightForBitMap(float skewness, int batchSize) {
  // collect all skew query names
  std::vector<std::string> skewBitMapQueryNames;
  for (auto const &queryNameIt: skewBitMapQueryNameMap_) {
    skewBitMapQueryNames.emplace_back(queryNameIt.first);
  }
  ...
  // random queries
  std::uniform_int_distribution<int> distribution2(1, skewBitMapQueryNames.size());
  ...
  for (size_t kind = 0; kind < possibilities.size(); kind++) {
    ...
    for (int i = 0; i < numThisKind; i++) {
      int skewWeightQueryIndex = distribution2(*generator_);
      auto skewWeightQueryName = skewBitMapQueryNames[skewWeightQueryIndex-1];
      queries.emplace_back(generateSqlSkewWeightForBitMap(skewWeightQueryName, skewLo_predicate, i % 2 == 0, testForBitMap));
    }
  }

  std::cout<<"query5 Num:"<<testForBitMap<<std::endl;
  if (queries.size() == batchSize) {
    return queries;
  } else if (queries.size() > batchSize) {
    return std::vector<std::string>(queries.begin(), queries.begin() + batchSize);
    ...
    while (queries.size() < batchSize) {
      ...
      int skewWeightQueryIndex = distribution2(*generator_);
      auto skewWeightQueryName = skewBitMapQueryNames[skewWeightQueryIndex-1];
      queries.emplace_back(generateSqlSkewWeightForBitMap(skewWeightQueryName, skewLo_predicate, true, testForBitMap));
    }
    std::cout<<"query5 Final Num:"<<testForBitMap<<std::endl;
    return queries;
  }
}

std::string SqlGenerator::generateSqlSkewWeightForBitMap(std::string queryName, std::string skewLo_predicate, bool high, int& testForBitMap){
  enum SkewBitMapQueryName queryNameEnum;
  auto queryNameIt = skewBitMapQueryNameMap_.find(queryName);
  if (queryNameIt == skewBitMapQueryNameMap_.end()) {
    queryNameEnum = SkewBitMapUnknown;
  } else {
    queryNameEnum = queryNameIt->second;
  }
  ...
  int kind = distributionForQuery5(*generator_);
  std::string skewLo_predicateForQuery5 = fmt::format("lo_commitdate = {}", (1990 + kind) * 10000 + 131);
  //std::string skewLo_predicateForQuery5 = fmt::format("(lo_orderdate between {} and {})", (1992 + kind) * 10000 + 101,
  //                                             (1992 + kind) * 10000 + 1231);

  switch (queryNameEnum) {
    case SkewBitMapQuery5: 
      testForBitMap++;
      return genSkewWeightQuery5(skewLo_predicateForQuery5);
    case SkewBitMapQuery2: return genSkewWeightQuery2(skewLo_predicate, lo_predicate, aggColumn);
    case SkewBitMapQuery3: return genSkewWeightQuery3(skewLo_predicate, lo_predicate, aggColumn);
    case SkewBitMapQuery4: return genSkewWeightQuery4(skewLo_predicate, lo_predicate, aggColumn);
    default:
      throw std::runtime_error("Unknown skew query name: " + queryName);
  }
}

std::vector<std::string> SqlGenerator::generateSqlBatchSkewWeightForDP(float skewness, int batchSize) {
  // collect all skew query names
  std::vector<std::string> skewDPQueryNames;
  for (auto const &queryNameIt: skewDPQueryNameMap_) {
    skewDPQueryNames.emplace_back(queryNameIt.first);
  }
  ...
  // random queries
  std::uniform_int_distribution<int> distribution2(1, skewDPQueryNames.size());

  std::vector<std::string> queries;
  int testForQuery0Num=0;
  ...
  for (size_t kind = 0; kind < possibilities.size(); kind++) {
    ...
    for (int i = 0; i < numThisKind; i++) {
      int skewWeightQueryIndex = distribution2(*generator_);
      auto skewWeightQueryName = skewDPQueryNames[skewWeightQueryIndex-1];
      queries.emplace_back(generateSqlSkewWeightForDP(skewWeightQueryName, skewLo_predicate, i % 2 == 0, testForQuery0Num));
    }
  }

  std::cout<<"query0 Num:"<<testForQuery0Num<<std::endl;
  ...
  if (queries.size() == batchSize) {
    ...
    while (queries.size() < batchSize) {
      ...
      int skewWeightQueryIndex = distribution2(*generator_);
      auto skewWeightQueryName = skewDPQueryNames[skewWeightQueryIndex-1];
      queries.emplace_back(generateSqlSkewWeightForDP(skewWeightQueryName, skewLo_predicate, true, testForQuery0Num));
    }
    std::cout<<"query0 Num fianl:"<<testForQuery0Num<<std::endl;
    return queries;
  }
}
...
std::string SqlGenerator::generateSqlSkewWeightForDP(std::string queryName, std::string skewLo_predicate, bool high, int& testForQuery0Num) {
  enum SkewDPQueryName queryNameEnum;
  auto queryNameIt = skewDPQueryNameMap_.find(queryName);
  if (queryNameIt == skewDPQueryNameMap_.end()) {
    queryNameEnum = SkewDPUnknown;
  } else {
    queryNameEnum = queryNameIt->second;
  }
  ...
  //std::uniform_int_distribution<int> distribution_high(low_orderkey+1,61000000);
  //int high_orderkey = distribution_high(*generator_);
  // int high_orderkey = low_orderkey+300;
  // assert(high_orderkey >= low_orderkey);

  switch (queryNameEnum) {
    case SkewDPQuery0: 
      testForQuery0Num++;
      //std::cout<<"test0"<<std::endl;
      return genSkewWeightQuery0(low_orderkey);
    case SkewDPQuery2: 
      //std::cout<<"test2"<<std::endl;
      return genSkewWeightQuery2(skewLo_predicate, lo_predicate, aggColumn);
    case SkewDPQuery3: 
      //std::cout<<"test3"<<std::endl;
      return genSkewWeightQuery3(skewLo_predicate, lo_predicate, aggColumn);
    case SkewDPQuery4: 
      //std::cout<<"test4"<<std::endl;
      return genSkewWeightQuery4(skewLo_predicate, lo_predicate, aggColumn);
    default:
      throw std::runtime_error("Unknown skew query name: " + queryName);
  }
}
...
std::string SqlGenerator::genSkewWeightQuery0(int low_orderkey){
  return fmt::format(
	  "select lo_orderdate\n"
	  "from lineorder\n"
	  "where lo_orderkey = {}\n",
    std::to_string(low_orderkey)
  );
}

std::string SqlGenerator::genSkewWeightQuery5(std::string skewLo_predicate){
  return fmt::format(
	  "select lo_orderdate, lo_linenumber\n"
	  "from lineorder\n"
	  "where {}\n",
    skewLo_predicate
  );
