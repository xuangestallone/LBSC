...
void generateLearningSegmentKeyAndSqlQueryMappings(const std::shared_ptr<Mode>& mode, const std::shared_ptr<LearningCachingPolicy>& learningCachingPolicy,
                                                 const std::string& bucket_name, const std::string& dir_prefix, int numQueries, std::filesystem::path sql_file_dir_path){
  Interpreter i(mode, learningCachingPolicy, false);
  configureS3ConnectorMultiPartition(i, bucket_name, dir_prefix);
  i.boot();
  int totalSegmentNum=0;
  int querySegmentNum = 0;
  ...
  for (auto queryNum = 1; queryNum <= numQueries; ++queryNum) {
    ...
    for (const auto& logicalOp: *logicalOperators) {
      ...
      for (const auto& segmentKey: *involvedSegmentKeys) {
        beladyLearningMiniCatalogue->addToSegmentQueryNumMappings(queryNum, segmentKey);
        totalSegmentNum++;
        querySegmentNum++;
      }
    }
    i.getOperatorGraph().reset();
    std::cout<<"query:"<<queryNum<<"   querySegmentNum:"<<querySegmentNum<<std::endl;
    sql_file_dir_path = sql_file_dir_path.parent_path();
  }

  std::cout<<"totalSegmentKey Num:"<<totalSegmentNum<<std::endl;
}

void generateBeladySCSegmentKeyAndSqlQueryMappings(const std::shared_ptr<Mode>& mode, const std::shared_ptr<BeladySizeCostCachingPolicy>& beladySCCachingPolicy,
                                                 const std::string& bucket_name, const std::string& dir_prefix, int numQueries, std::filesystem::path sql_file_dir_path){
  Interpreter i(mode, beladySCCachingPolicy, false);
  configureS3ConnectorMultiPartition(i, bucket_name, dir_prefix);
  i.boot();
  ...
  for (auto queryNum = 1; queryNum <= numQueries; ++queryNum) {
    ...
    for (const auto& logicalOp: *logicalOperators) {
      auto involvedSegmentKeys = logicalOp->extractSegmentKeys();
      for (const auto& segmentKey: *involvedSegmentKeys) {
        beladySCMiniCatalogue->addToSegmentQueryNumMappings(queryNum, segmentKey);
      }
    }
    i.getOperatorGraph().reset();
    sql_file_dir_path = sql_file_dir_path.parent_path();
...
void generateBeladySegmentKeyAndSqlQueryMappings(const std::shared_ptr<Mode>& mode, const std::shared_ptr<BeladyCachingPolicy>& beladyCachingPolicy,
                                                 const std::string& bucket_name, const std::string& dir_prefix, int numQueries, std::filesystem::path sql_file_dir_path) {
  Interpreter i(mode, beladyCachingPolicy, false);
  configureS3ConnectorMultiPartition(i, bucket_name, dir_prefix);
  i.boot();
