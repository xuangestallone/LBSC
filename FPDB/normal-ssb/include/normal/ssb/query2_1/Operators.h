...
#ifndef NORMAL_NORMAL_SSB_INCLUDE_NORMAL_SSB_QUERY2_1_OPERATORS_H
#define NORMAL_NORMAL_SSB_INCLUDE_NORMAL_SSB_QUERY2_1_OPERATORS_H
...
public:
  ...
  makeSupplierFilterOperators(const std::string &region,
							  int numConcurrentUnits,
							  const std::shared_ptr<OperatorGraph> &g);

  static std::vector<std::shared_ptr<normal::pushdown::filter::Filter>>
  makePartFilterOperators(const std::string &category,
						  int numConcurrentUnits,
						  const std::shared_ptr<OperatorGraph> &g);
...
#endif //NORMAL_NORMAL_SSB_INCLUDE_NORMAL_SSB_QUERY2_1_OPERATORS_H
