...
S3SelectScanLogicalOperator::toOperatorsHybridCaching(int numRanges) {
  ...
  std::shared_ptr<std::vector<std::shared_ptr<Partition>>> partitions = getPartitioningScheme()->partitions();
  //std::shared_ptr<std::vector<std::shared_ptr<Partition>>> finalPartitions;
  int partitionSize=partitions->size();
  std::vector<bool> bForDP(partitionSize, false);
  std::vector<std::pair<int,int>> vKeyDate;
  if(normal::plan::Planner::getDP()){
    checkPartitionValidForDP(partitions, bForDP);
  }
  // }else{
  //   finalPartitions = partitions;
  // }
  int DPFilterPartitionNum=0;
  for(int i=0;i<bForDP.size();i++){
    if(bForDP[i]){
      DPFilterPartitionNum++;
    }
  }

  if(normal::plan::Planner::getBitmap()){
    checkPartitionValidForBitMap(partitions, bForDP);
  }
  DPFilterPartitionNum=0;
  for(int i=0;i<bForDP.size();i++){
    if(bForDP[i]){
      DPFilterPartitionNum++;
    }
  ...
  for (const auto &partition: *getPartitioningScheme()->partitions()) {
  //for (const auto &partition: *finalPartitionsAfterBitMap) {
    // Check if valid for predicates (if will get empty result), and extract only useful predicates (can at least filter out some)
    if(bForDP[partitionIndex]){
      partitionIndex++;
      continue;
    }
    ...
    int rangeId = 0;
    for (const auto &scanRange: scanRanges) {
      // weighted segment keys   predicate column+  project column
      auto weightedSegmentKeys = std::make_shared<std::vector<std::shared_ptr<normal::cache::SegmentKey>>>();
      for (auto const &projectedColumnName: *projectedColumnNames_) {
        weightedSegmentKeys->emplace_back(normal::cache::SegmentKey::make(s3Partition, projectedColumnName,
                                                                          SegmentRange::make(scanRange.first, scanRange.second)));
  ...
  // std::cout<<"InvalidNum:"<<InvalidNum<<"  opearators.size():"<<operators->size()<<std::endl;
