...
void LRBCache::train() {
    ...
    training_time  += chrono::duration_cast<chrono::microseconds>(chrono::system_clock::now() - timeBegin).count();
}
...
void LRBCache::update_stat_periodic() {
    ...
    cerr
            ...
            << "training_add_feature_time: " << training_add_feature_time/1000 << " ms" << endl
            ...
            << "obj_distribution ratio: " << percent_beyond << endl
            << "training_data_distribution ratio: "<< positive_example_ratio << endl
    ...
    cerr<<endl;
    while(training_losses.size()>0){
        training_losses.pop_back();
    }

    n_retrain = 0;
    training_time  = 0;
    training_add_feature_time = 0;
...
bool LRBCache::lookup(const SimpleRequest &req) {
    ...
    if (it != key_map.end()) {
        ...
        if (!meta._sample_times.empty()) {
            auto addFeatureTimeBegin = chrono::system_clock::now();
            //mature
            ...
            training_add_feature_time += chrono::duration_cast<chrono::microseconds>(chrono::system_clock::now() - addFeatureTimeBegin).count();
        }
...
pair<uint64_t, uint32_t> LRBCache::rank() {
    ...
    inference_add_feature_time  += chrono::duration_cast<chrono::microseconds>(chrono::system_clock::now() - addFeatureTimeBegin).count();
