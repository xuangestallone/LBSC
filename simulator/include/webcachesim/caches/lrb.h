...
struct MetaExtra {
    ...
    uint8_t _past_distance_idx = 1; //1
    ...
    void init_with_params(const map<string, string> &params) override {
        for (auto &it: params) {
            if (it.first == "sample_rate") {
                ...
                cerr << "byte_million_req: " << byte_million_req << endl;
#ifdef EVICTION_LOGGING
...
//        inference_params["num_threads"] = "4";
    ...
    static void set_hash_edc() {
        max_hash_edc_idx = (uint64_t) (memory_window / pow(2, base_edc_window)) - 1; // memory_window除以2^10 -1 
