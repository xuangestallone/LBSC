...
namespace webcachesim {
    ...
    public:
        ...
        virtual void setInters(
            unordered_map<uint64_t, vector<uint64_t>>& inters, 
            unordered_map<uint64_t, uint64_t>& counter, 
            unordered_map<uint64_t,uint64_t>& sizes,
            unordered_map<uint64_t, double>& avg,
            unordered_map<uint64_t, uint64_t>& cost
        ){
            cerr<<"test setInters"<<endl;
        }

        virtual void setReqNum(uint64_t req_num, uint64_t max_data_size){
            cerr<<"test setReqNum"<<endl;
        }
